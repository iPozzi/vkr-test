...
export default function AdminTagsPage() {
  ...
  useEffect(() => {
    // Загрузка списка тегов при монтировании компонента
    const fetchTags = async () => {
      try {
        ...
        setError(err instanceof Error ? err.message : "Произошла ошибка при загрузке тегов");
        console.error(err);
      } finally {
        setIsLoading(false);
      }
  ...
  // Фильтрация тегов на основе строки поиска
  ...
  const handleAddTag = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setModalError(err instanceof Error ? err.message : "Произошла ошибка при создании тега");
      console.error(err);
    } finally {
  ...
  const handleEditTag = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/admin/tags", {
        ...
        body: JSON.stringify({ id: editTag.id, name: editTag.name })
      });

      const data = await response.json();

      if (!response.ok) {
        throw new Error(data.message || "Не удалось обновить тег");
      }
      ...
      setModalError(err instanceof Error ? err.message : "Произошла ошибка при обновлении тега");
      console.error(err);
    } finally {
      setModalLoading(false);
    }
  ...
  const handleDeleteTag = async () => {
    ...
    try {
      const response = await fetch(`/api/admin/tags?id=${deleteTagId}`, {
        method: "DELETE"
      });
      ...
      setModalError(err instanceof Error ? err.message : "Произошла ошибка при удалении тега");
      console.error(err);
    } finally {
      setModalLoading(false);
    }
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
        ...
        <>
          {filteredTags.length === 0 ? (
            <div className="text-center text-gray-500 my-12">
              {tags.length === 0 ? "Теги отсутствуют в базе данных" : "Нет тегов, соответствующих запросу"}
            </div>
          ) : (
              ...
              <table className="w-full divide-y divide-gray-700">
                ...
                <tbody className="bg-gray-900 divide-y divide-gray-700">
                  {filteredTags.map((tag) => (
                    <tr key={tag.id} className="hover:bg-gray-800">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div className="flex justify-end gap-3">
                          <button
                            ...
                            aria-label={`Редактировать ${tag.name}`}
                          >
                            <Edit size={18} />
                          </button>
                          <button
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-gray-800 rounded-lg p-6 max-w-md w-full">
            <h3 className="text-xl font-bold mb-4">Добавить новый тег</h3>
            ...
            <form onSubmit={handleAddTag}>
              <div className="mb-4">
                <label htmlFor="tagName" className="block mb-2 text-sm font-medium">
                  Название тега
                </label>
                <input
                  type="text"
      ...
      {showEditModal && editTag && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-gray-800 rounded-lg p-6 max-w-md w-full">
            <h3 className="text-xl font-bold mb-4">Редактировать тег</h3>
            ...
            <form onSubmit={handleEditTag}>
              <div className="mb-4">
                <label htmlFor="editTagName" className="block mb-2 text-sm font-medium">
                  Название тега
                </label>
                <input
                  type="text"
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-gray-800 rounded-lg p-6 max-w-md w-full">
            ...
            <p className="mb-6">Вы уверены, что хотите удалить этот тег? Это действие нельзя отменить.</p>
